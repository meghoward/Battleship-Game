...
class AutomaticPlayer(Player):
    ...
    def select_target(self):
        ...
        # NOTE: Please check if this is off the edge of the board or otherwise in the possible squares 
        if len(self.successful_hit_coordinates) == 1:
            while self.target_coordinates not in self.possible_cells: 
                # choose_direction = random.choice(['horizontal', 'vertical']) == 'horizontal'
                if (self.successful_hit_coordinates[0][0] - 1, self.successful_hit_coordinates[0][1]) in self.possible_cells:
                    self.target_coordinates = (self.successful_hit_coordinates[0][0] - 1, self.successful_hit_coordinates[0][1])

                elif (self.successful_hit_coordinates[0][0] + 1, self.successful_hit_coordinates[0][1]) in self.possible_cells:
                    self.target_coordinates = (self.successful_hit_coordinates[0][0] + 1, self.successful_hit_coordinates[0][1])

                elif (self.successful_hit_coordinates[0][0], self.successful_hit_coordinates[0][1] - 1) in self.possible_cells:
                        self.target_coordinates = (self.successful_hit_coordinates[0][0], self.successful_hit_coordinates[0][1] - 1)
    ...
    # def get_4_neighbours(self, cell):
    #     x,y = cell
    #     neighbours = []

    #     for x_coord in [-1,1]:
    #         for y_coord in [-1,1]:
    #             neighbouring_x_up, neighbouring_y+ = x + x_coord, y + y_coord
    #             if 1<= neighbouring_x <= self.board.width and 1 <= neighbouring_y <= self.board.height:
    #                 neighbours.append((neighbouring_x, neighbouring_y))
